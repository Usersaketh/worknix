...
export default function JobsPrivate() {
  ...
  useEffect(() => { (async ()=> { setLoading(true); setError(null); try { const data = await getJobs('private'); setJobs(data); toast({ title: 'Loaded', description: `${data.length} private jobs.` }); } catch(e: unknown){ const msg = (e as { message?: string })?.message || 'Failed to load'; setError(msg); toast({ title: 'Load failed', description: msg, variant: 'destructive' }); } finally { setLoading(false);} })(); }, []);

  const orgOptions = useMemo(() => {
    const set = new Set<string>();
    jobs.filter(j => j.kind==='private').forEach(j => set.add(j.org));
  ...
  const filtered = useMemo(() => {
    ...
    return jobs.filter(j => {
      ...
      if (j.deadline) {
        ...
        if (deadlineDate < now && (now.getTime() - deadlineDate.getTime()) > 24 * 60 * 60 * 1000) expired = true; // >24h past deadline
      }
    ...
    <div className="min-h-screen bg-secondary/20 pt-6">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <Card className="mb-8 shadow-[var(--shadow-card)]">
          <CardHeader>
            <CardTitle>Private Job Listings</CardTitle>
            <CardDescription>All currently posted private sector roles.</CardDescription>
          </CardHeader>
            ...
            <>
              ...
              {import.meta.env.VITE_ENABLE_ADS === 'true' && (idx+1) % 6 === 0 && (
                <div className="sm:col-span-2 lg:col-span-3" key={`ad-${j.id}`}>
                  <AdSlot slot={import.meta.env.VITE_ADSENSE_SLOT_JOBS || '0000000000'} />
                </div>
              )}
            </>
          ))}
